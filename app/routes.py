  ...
  Tareas detalladas:"""
    ...
    if any(
        ...
        if family_data["usuarios"]:
            max_tasks_user = max(
                family_data["usuarios"], key=lambda u: u["total_tareas"]
            )
            min_tasks_user = min(
                family_data["usuarios"], key=lambda u: u["total_tareas"]
            )
...
class SearchCache:
    ...
    def set(self, query, data, filters=None):
        ...
        with self.lock:
            ...
            logger.info(
                f"💾 Cache SAVE para búsqueda: '{query}' ({len(data.get('products', []))} productos)"
            )
...
def index():
    ...
    try:
        ...
        return render_template(
            "index.html", users=users, vapid_public_key=vapid_public_key, stats=stats
        )
    except Exception as e:
...
# Caché de 2 mins
# ==========================================
_mercadona_categories_cache = {"data": None, "timestamp": None}
...
def tareas():
    ...
    try:
        ...
        stats = {"total_tasks": total_tasks, "overdue_tasks": overdue_tasks}

        return render_template(
            "tareas.html", users=users, vapid_public_key=vapid_public_key, stats=stats
        )
    except Exception as e:
...
def add_task():
    try:
        ...
        mongo.db.users.update_one(
            {"_id": user["_id"]}, {"$push": {"tareas": nueva_tarea}}
        )
...
def mostrar_menus():
    ...
    try:
        ...
        total_menus = sum(
            1
            for dia_data in menus_data.values()
        ...
        return render_template("menus.html", menus=menus_data, users=users, stats=stats)
    except Exception as e:
...
def add_menu():
    ...
    try:
        ...
        existing = mongo.db.menus.find_one({"dia": dia, "momento": momento})
...
def reset_menus():
    ...
    try:
        ...
        return jsonify({"status": "reseteado", "deleted_count": result.deleted_count})
    except Exception as e:
...
def asignar_comida():
    ...
    try:
        ...
        mongo.db.users.update_many(
            {}, {"$pull": {"tareas": {"titulo": tarea_titulo, "due_date": fecha_real}}}
        )
        ...
        mongo.db.users.update_one(
            {"nombre": miembro}, {"$push": {"tareas": nueva_tarea}}
        )

        return jsonify(
            {"status": "ok", "tarea_creada": True, "tarea_titulo": tarea_titulo}
        )
    except Exception as e:
...
def lista_compra():
    ...
    try:
        ...
        return render_template("lista_compra.html", items=items, stats=stats)
    except Exception as e:
...
def configuracion():
    ...
    try:
        ...
        if previous_tasks > 0:
            tasks_trend = round(
                ((current_tasks - previous_tasks) / previous_tasks) * 100, 1
            )
        ...
        if previous_users > 0:
            users_trend = round(
                ((len(users) - previous_users) / previous_users) * 100, 1
            )
        ...
        for i in range(7, 0, -1):
            ...
            count = mongo.db.completed_tasks.count_documents(
                {"completion_date": {"$gte": day, "$lt": next_day}}
            )
        ...
        return render_template(
            "configuracion.html", users=users, system_stats=system_stats
        )
    except Exception as e:
...
def chat():
    ...
    try:
        ...
        stats = {"total_messages": total_messages, "today_messages": today_messages}
...
def test_push(username):
    ...
    try:
        ...
        if not subscripcion:
            return (
                jsonify({"error": f"No se encontró suscripción para {username}"}),
                404,
            )
        ...
        for sub in subscripcion.get("subscriptions", []):
            try:
                ...
                return (
                    jsonify({"error": "Fallo al enviar push", "details": str(ex)}),
                    500,
                )
...
def generate_enhanced_ai_context(family_data, user_query):
    ...
    if any(
        ...
        try:
            ...
            existing_shopping = family_data.get("lista_compra", {}).get(
                "items_detalle", []
            )
...
Responde de manera práctica y específica.""",
            ...
            session["chat_history"][0][
                "content"
            ] = f"""Eres 'Casa AI', un asistente familiar inteligente especializado en optimización doméstica.
...
Responde de manera práctica y específica."""
        ...
        additional_data = {
            ...
            "completed_today": family_data.get("estadisticas", {}).get(
...
def mercadona_categories():
    ...
    if (
        ...
        and _mercadona_categories_cache["timestamp"]
        and (datetime.now() - _mercadona_categories_cache["timestamp"]).total_seconds()
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            if _mercadona_categories_cache["data"]:
                return jsonify(_mercadona_categories_cache["data"])
            return (
                jsonify({"success": False, "error": "Error al conectar con Mercadona"}),
                500,
            )
        ...
        try:
            ...
            if _mercadona_categories_cache["data"]:
                return jsonify(_mercadona_categories_cache["data"])
            return (
                jsonify({"success": False, "error": "Respuesta inválida de Mercadona"}),
                500,
            )
        ...
        if not isinstance(results, list):
            if _mercadona_categories_cache["data"]:
                return jsonify(_mercadona_categories_cache["data"])
            return (
                jsonify({"success": False, "error": "Formato de datos inválido"}),
                500,
            )
...
def get_products_from_subcategory(session, subcategory_id):
    ...
    try:
        subcat_url = (
            f"{MERCADONA_BASE_URL}/categories/{subcategory_id}/?lang=es&wh=mad1"
        )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            formatted_product = {
                ...
                "price": product_data.get("price_instructions", {}).get(
                    "unit_price", "0"
                ),
                "reference_price": product_data.get("price_instructions", {}).get(
                    "reference_price", "0"
                ),
                "size": product_data.get("price_instructions", {}).get("unit_size", 1),
                "size_format": product_data.get("price_instructions", {}).get(
                    "size_format", ""
                ),
                ...
                "ingredients": product_data.get("nutrition_information", {}).get(
                    "ingredients", ""
                ),
                "storage": product_data.get("details", {}).get(
        ...
        return (
            jsonify(
                {"success": False, "error": "Error al obtener detalles del producto"}
            ),
            500,
        )
...
def format_mercadona_product(product_data):
    ...
    return {
        ...
        "thumbnail": safe_string(
            product_data.get("thumbnail"), "/static/img/default_food.jpg"
        ),
...
def mercadona_search():
    ...
    try:
        ...
        if cached_result:
            ...
            logger.info(
                f"🎯 Cache HIT: '{query}' ({cached_result.get('total_found', 0)} productos)"
            )
        ...
        total_subcategories = sum(
            len(cat.get("categories", [])) for cat in categories_data.get("results", [])
        )
        processed_count = 0
        error_count = 0
        success_count = 0
            ...
            for subcat in category.get("categories", []):
                ...
                if subcat_id:
                    try:
                        ...
                        subcat_products = get_subcategory_products_cached(
                            session_search, subcat_id, subcat_name
                        )
                        ...
                        if subcat_products:  # Solo procesar si hay productos
                            ...
                            for product in subcat_products:
                                if is_product_match(
                                    product, query_normalized, query_words
                                ):
                                    try:
                                        formatted_product = format_mercadona_product(
                                            product
                                        )
                                        ...
                                        if current_app.debug:
                                            logger.debug(
                                                f"Error formateando producto {product.get('id', 'unknown')}: {str(e)}"
                                            )
                                        continue
                        ...
                        if "403" not in str(e) and "NoneType" not in str(e):
                            logger.warning(
                                f"Error procesando subcategoría {subcat_id}: {str(e)}"
                            )
                        continue
        ...
        sorted_products = sort_products_by_relevance(
            unique_products, query_normalized, query_words
        )
        ...
        # Log final conciso
        logger.info(
            f"✅ '{query}': {len(sorted_products)} productos, {search_duration:.2f}s, {success_count}/{processed_count} subcategorías OK"
        )
            ...
            jsonify(
                {
                    ...
                    "details": (
                        str(e) if current_app.debug else "Error interno del servidor"
                    ),
                }
            ),
            500,
...
def get_mercadona_categories_cached(session):
    ...
    # Si hay cache y no ha caducado
    if (
        _mercadona_categories_cache.get("data")
        and _mercadona_categories_cache.get("timestamp")
        and (datetime.now() - _mercadona_categories_cache["timestamp"]).total_seconds()
        < _CATEGORIES_CACHE_TTL
    ):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(
                f"✅ {len(categories_data.get('results', []))} categorías obtenidas"
            )
...
def get_subcategory_products_cached(session, subcat_id, subcat_name):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Procesar productos directos
            for product in subcat_data.get("products", []):
                if product and isinstance(
                    product, dict
                ):  # Validar que el producto no es None
            ...
            if current_app.debug:
                logger.debug(
                    f"HTTP {response.status_code} para subcategoría {subcat_id}"
                )
            return []
...
def planes():
    ...
    try:
        ...
        return render_template("planes.html", theme=theme, current_page="planes")
...
def move_plan_to_top(plan_id):
    ...
    try:
        ...
        max_priority_plan = mongo.db.planes.find_one(
            {"user_id": current_user.id}, sort=[("prioridad", -1)]
        )
